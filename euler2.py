...
def max_five():
    ...
    for i in range(0, len(s)-5+1):
        lst.append(reduce(lambda x, y: x * y, map(int, s[i:i+5])))
    return max(lst)
...
def factorial():
    f = lambda n: reduce(lambda x, y: x*y, range(n, 1, -1))

    return sum(int(i) for i in str(f(100)))
